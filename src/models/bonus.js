...
import { BonusRequestStatus, BonusStatus } from '../constants/constants';
...
const addBonus = async (text, content, status) => {
  await DB.executeQuery(
    'insert into bonuses (bonus_text,bonus_content,bonus_status) VALUES ($1,$2,$3)',
    [text, content, status]
  );
};
...
const updateBonus = async (id, text, content, status) => {
  await DB.executeQuery(
    'update bonuses set bonus_text = $1, bonus_content = $2, bonus_status = $3 where id = $4',
    [text, content, status, id]
  );
};

const getList = async () => {
  const { rows } = await DB.executeQuery(`${selectBonusSql}`, [
...
const getActiveList = async () => {
  const { rows } = await DB.executeQuery(
    `select id, bonus_text, bonus_content from bonuses where bonus_status = $1`,
    [BonusStatus.ACTIVE]
  );
  return rows;
};
...
const getBonusById = async (id) => {
  const {
    rows: [result],
  } = await DB.executeQuery(`${selectBonusSql} where id = $2`, [
    BonusRequestStatus.ACCEPTED,
    id,
  ]);
  return result;
...
const getBonusPermissionsByUserId = async (userId) => {
  const { rows } = await DB.executeQuery(
    'select bonus_id as id from user_bonus_permissions where user_id = $1',
    [userId]
  );
